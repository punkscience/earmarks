 ...
 *
 * For dirplay earmarks the sender and recipient are the same key (self-encryption).
 */
object Nip44 {
    ...
    fun decrypt(privKeyHex: String, encryptedContent: String): String {
        ...
        val decrypted = chacha20Decrypt(chachaKey, chachaNonce, ciphertext)
    ...
    private fun chacha20Decrypt(key: ByteArray, nonce: ByteArray, ciphertext: ByteArray): ByteArray {
        ...
        return ByteArray(ciphertext.size).also { out ->
            engine.processBytes(ciphertext, 0, ciphertext.size, out, 0)
        }
    }
}
